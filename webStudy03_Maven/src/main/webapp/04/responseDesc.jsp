...
<pre>
	...
	1)Response Line : Response Status Code, Protocol/ver
	  ...
	  -400 : Client쪽 문제로 처리 실패
	  	...
	  	401(UnAuthorized), 403(Forbidden)->보안처리에서 많이 사용된다
	  -500 : Server쪽 문제로 처리 실패
	  	500(Internal Server Error)
	2)Response Header : 서버나 응답데이터에 대한 부가정보(metadata)
		response header name : header value
		
	3)Response Body(Message Body) : 응답컨텐츠
	...
	**HttpServletResponse를 통한 응답제어
	...
	2)setStatus(status_code) : 200/300사용
	  sendError(status_code) : 400/500사용, 서버의 에러페이지로 자동연결.
...
// 	  	response.setDateHeader("Expires", 0);//long이면 밀리세컨드 int면 세컨드
	  //캐시를 저장하도록 설정
