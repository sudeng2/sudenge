...
<head>
<meta charset="UTF-8">
<title>04/04/calendar.jsp</title>
<style type = "text/css">
	.sunday{
		color : red;
	}
...
<%
	...
	int currentYear = cal.get(YEAR);/*  몇년몇월인지 꺼내야함 */
	...
	cal.add(MONTH, -1); /*날짜연산, 월에서 1을 차감하겠다. amount는 전 달이니까 -1 */
...
<form>
<h4>
<a href = "<%=request.getContextPath()%>/04/Calendar.jsp?year=<%=beforeYear%>&month=<%=beforeMonth%> &language=<%=clientLocale%>">이전달</a>
&nbsp;&nbsp;&nbsp;&nbsp;
<%-- <%=String.format("%d년 %d월", currentYear, currentMonth+1) %> --%>
...
&nbsp;&nbsp;&nbsp;&nbsp;
<a href = "?year=<%=nextYear %>&month=<%=nextMonth %>&language=<%=clientLocale%>">다음달</a>
</h4>
</form>
<table>
<thead>
