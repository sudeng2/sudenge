...
<pre>
	...
	세션??(시간과 통로)
		...
		사용 시작 : 클라이언트의 브라우저로부터 최초의 요청이 발생했을 때 -> session객체 생성
		사용 종료 : 
			1)명시적인 로그아웃
			2)세션 만료 시간 이내에 새로운 요청이 발생하지 않을때
			3)브라우저를 완전히 종료한 경우
			
		세션 아이디:<%=session.getId() %>	
		세션 생성 시점 : <%=new Date(session.getCreationTime()) %>
		가장 마지막 접속시간 : <%=new Date(session.getLastAccessedTime()) %>
		...
		<a href = "sessionDesc.jsp;jsessionid = <%=session.getId()%>">쿠키 없는 상태에서 세션유지</a>
</pre>
</body>
</html>