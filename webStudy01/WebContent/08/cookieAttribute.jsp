...
<pre>
	...
	2.domain/host : 저장된 쿠키의 재전송 여부를 결정할 때 사용,
					...
					도메인의 레벨 구조에 따라 ".naver.com방식으로 설정함"
	3.maxAge : 만료시간, 생략하면 기본값으로 세션의 만료시간이 반영됨.	
			-1: 브라우저의 종료 시점에 삭제.
			 0: 기존의 쿠키가 있을경우, 해둥 쿠키를 삭제할때 사용.
			 (maxAge를 제외한 나머지 설정이 모두 동일한 경우)
	4.secure : 보안 채널을(https)을 통해서만 전송될 쿠키를 설정.
	5.name/value(required) : RFC2396 규약에따라 URL Encoding방식이 적용됨.
	<%
...
//  		otherDomainCookie.setDomain(".naver.com");
//  		otherDomainCookie.setPath("/");
//  		response.addCookie(otherDomainCookie);
			
			Cookie longLiveCookie = new Cookie("longLiveCookie","LongLive Cookie");
